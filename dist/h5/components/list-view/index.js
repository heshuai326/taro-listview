...
class ListView extends Taro.Component {
  ...
  componentDidMount() {
    this.moveBox(0);
    if (this.props.lazy) {
      _createSelectorQuery().in(this.$scope).select(".scrollView").boundingClientRect().exec(res => {
  ...
  render() {
    ...
    if (autoHeight) {
      return <ScrollView ref={node => {
        this.scrollView = node;
      }} className={`${className} ${downLoading && "downLoadingStyle"} scrollView autoHeight`} style={{ ...style }} scrollY={canScrollY} lowerThreshold={80} onScrollToLower={this.handleScrollToLower} scrollWithAnimation onScroll={this.onScroll}>
          <View style={{ minHeight: "100%", overflowY: "hidden" }} onTouchMove={e => this.touchEvent(e)} onTouchEnd={e => this.touchEvent(e)} onTouchStart={e => this.touchEvent(e)} onTouchCancel={e => this.touchEvent(e)}>
            <View
          // style={trStyle}
          className="bodyView" id="bodyView">
              <View style={blockStyle}>
                <View style={{ height: `${damping}px`, marginTop: `-${damping}px` }} className="pullDownBlock">
                  <View className="tip">
                    ...
                    {downLoading && (this.props.customizeLoading ? this.props.renderCustomizeLoading : <Loading color={circleColor} />)}
                  </View>
                </View>
                ...
                {footerLoading && <View className="loading">{footerLoadingText}</View>}
                
                {customFooterLoading && this.props.renderFooterLoading}
                
                {footerLoaded && <View className="loaded">{footerLoadedText}</View>}
  ...
  moveBox = y => {
    ...
    // console.log({ y })
    if (Taro.getEnv() === "WEB") {
      const target = document.getElementById("bodyView");
  ...
  updateDampText = act => {
    ...
    if (Taro.getEnv() === "WEB") {
      const target = document.getElementById("tip-dampText");
